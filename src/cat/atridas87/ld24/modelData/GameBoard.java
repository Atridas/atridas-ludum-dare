...
public final class GameBoard {
	...
	/*
	 * public static final int SKILL_DECK_MAIN_SKILL_CARD_COUNT = 5; public
	 * static final int SKILL_DECK_MAIN_X2_SKILL_CARD_COUNT = 10; public static
	 * final int SKILL_DECK_MAIN_X3_SKILL_CARD_COUNT = 3; public static final
	 * int SKILL_DECK_OTHER_SKILL_CARD_COUNT = 3; public static final int
	 * SKILL_DECK_MIXED_SKILL_CARD_COUNT = 3; public static final int
	 * SKILL_DECK_MIXED_X2_SKILL_CARD_COUNT = 1; public static final int
	 * SKILL_DECK_STAR_CARD_COUNT = 10;
	 */
	public static final int SKILL_DECK_MAIN_SKILL_CARD_COUNT = 10;
	public static final int SKILL_DECK_MAIN_X2_SKILL_CARD_COUNT = 20;
	public static final int SKILL_DECK_MAIN_X3_SKILL_CARD_COUNT = 6;
	public static final int SKILL_DECK_OTHER_SKILL_CARD_COUNT = 6;
	public static final int SKILL_DECK_MIXED_SKILL_CARD_COUNT = 6;
	public static final int SKILL_DECK_MIXED_X2_SKILL_CARD_COUNT = 2;
	public static final int SKILL_DECK_STAR_CARD_COUNT = 20;
	...
	public SkillCard drawSkill(float x, float y, float w, float h,
			float mouseX, float mouseY, Set<SkillColor> possibleColors) {
		...
		for (SkillColor color : possibleColors) {
			...
			float marginX = cardsOnDeck * hUnit / 200.f;
			float marginY = -cardsOnDeck * hUnit / 200.f;

			if (mouseX >= sDeckX + marginX && mouseY >= sDeckY + marginY
	...
	public void initGame() {
		...
		for (int i = 0; i < NUMBER_OF_PLAYERS; i++) {
			...
			for (int j = 0; j < INITIAL_NUMBER_OF_CREATURES; j++) {
				Creature choosenCreature = chooseCreatureAtRandom(
						chosenCreatures, rnd);
	...
	static Creature chooseCreatureAtRandom(Set<Creature> chosenCreatures,
			Random rnd) {
		int index = rnd.nextInt(Creature.values().length
				- chosenCreatures.size());
	...
	static ArrayList<SkillCard> createSkillDeck(SkillColor color) {
		...
		for (int i = 0; i < SKILL_DECK_MAIN_X2_SKILL_CARD_COUNT; i++) {
			deck.add(new SkillCard(color.mainAttribute, color.mainAttribute,
					color));
		}

		for (int i = 0; i < SKILL_DECK_MAIN_X3_SKILL_CARD_COUNT; i++) {
			deck.add(new SkillCard(color.mainAttribute, color.mainAttribute,
					color.mainAttribute, color));
		}
		...
		for (int i = 0; i < SKILL_DECK_MIXED_SKILL_CARD_COUNT; i++) {
			for (Attribute attribute : Attribute.values()) {
				if (attribute != color.mainAttribute) {
					deck.add(new SkillCard(color.mainAttribute, attribute,
							color));
				}
			}
		}
		...
		for (int i = 0; i < SKILL_DECK_MIXED_X2_SKILL_CARD_COUNT; i++) {
			for (Attribute attribute : Attribute.values()) {
				if (attribute != color.mainAttribute) {
					deck.add(new SkillCard(color.mainAttribute,
							color.mainAttribute, attribute, color));
				}
			}
		}
	...
	public void draw(float x, float y, float w, float h) {
		if (font == null) {
			try {
				font = new UnicodeFont("resources/Font/accid___.ttf", 16,
		...
		Image backSide = im.getEnvironmentBackSide();
		{
			...
			EnvironmentCard card : environmentDeck) {
		...
		font.drawString(hUnit * 3.33f, vUnit * 2.25f + mCardH, "Environment");
		{
			...
			float eDeckY = vUnit * 2;
			for (@SuppressWarnings("unused")
			EnvironmentCard card : environmentDiscardDeck) {
				backSide.draw(eDeckX, eDeckY, mCardW, mCardH);
				eDeckX += hUnit / 20;
				eDeckY -= hUnit / 20;
			}
...

		{
			...
			for (@SuppressWarnings("unused")
			EnvironmentCard card : combatDeck) {
				backSide.draw(eDeckX, eDeckY, mCardW, mCardH);
				eDeckX += hUnit / 20;
				eDeckY -= hUnit / 20;
			}
...

		{
			...
			float eDeckY = vUnit * 5;
			for (@SuppressWarnings("unused")
			EnvironmentCard card : combatDiscardDeck) {
				backSide.draw(eDeckX, eDeckY, mCardW, mCardH);
				eDeckX += hUnit / 20;
				eDeckY -= hUnit / 20;
			}
		...
		// skills
		for (Entry<SkillColor, ArrayList<SkillCard>> deck : skillDecks
				.entrySet()) {
			float sDeckX;
			float sDeckY = vUnit * 2;
			switch (deck.getKey()) {
